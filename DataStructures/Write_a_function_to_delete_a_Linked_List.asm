...
deleteList PROC
    ...
    mov eax, [ebp+8]
    mov eax, [eax]
    mov [ebp-4], eax; struct node *current = *head_ref
...
while_loop:
    ...
    je out_side  ; {
    mov ebx, [eax+4]
    mov [ebp-8], ebx; nex = current->next 

    mov eax, [ebp-4]
    push eax
    call crt_free
    add esp, 4; free(current);

    mov eax, [ebp-8]
    mov [ebp-4], eax; current = next
    jmp while_loop
out_side:  
    xor ecx, ecx
    mov eax, [ebp+8]
    mov [eax], ecx; *head_ref = NULL
...
main PROC
  ...
  ;It make no sense to print the list
